...
// v0.2.9 — build the body of a "smart" .stowignore: starts with
// DEFAULT_STOWIGNORE and, when `detected` is non-empty, appends an
// "# Auto-detected from secret-scan" section listing the detected paths
// converted from absolute to claude_dir-relative (POSIX-normalised) form.
// Pure function — no I/O — so unit tests exercise the path-strip + dedup
// logic without a real filesystem. Stable errors:
//   "path_outside_claude_dir" — detected path is not under claude_dir
fn build_smart_stowignore_body(
    default: &str,
    claude_dir: &Path,
    detected: &[String],
    ...
    for abs in detected {
        ...
        let rel = match p.strip_prefix(claude_dir) {
            Ok(r) => r.to_path_buf(),
            Err(_) => {
                // v0.2.9 hotfix: Windows에서 사이드카가 출력한 경로의 드라이브
                // 문자 대소문자가 resolve_claude_dir()와 다를 수 있다
                // (예: "c:\Users\..." vs "C:\Users\..."). Path::strip_prefix는
                // OsStr 비교라 case-sensitive — case-insensitive 비교로 폴백.
                let abs_str = p.to_string_lossy().to_lowercase();
                let claude_str = claude_dir.to_string_lossy().to_lowercase();
                // 경로 구분자도 통일해서 비교 (mixed separator 방어 — SUGGESTION 2)
    ...
    body.push_str("\n# Auto-detected from secret-scan\n");
    for r in &rels {
        body.push_str(r);
        body.push('\n');
    }
...
mod tests {
    ...
    fn create_stowignore_writes_default_body_when_missing() {
        ...
        // v0.2.9 — new DEFAULT entries; ensure the regression suite catches
        // any future drift on these patterns the v0.2.8 dogfood revealed.
        assert!(body.contains("session-data/\n"));
        assert!(body.contains("plugins/marketplaces/\n"));
    ...
    // v0.2.9 — build_smart_stowignore_body: pure path-strip + dedup helper
    // that lets the FE pipe secret-scan-detected paths into the generated
    // .stowignore. No I/O so we can exercise every branch in-process.

    #[test]
    fn build_smart_stowignore_body_empty_detected_returns_default() {
        let dir = make_tempdir("smart-empty");
        let body = build_smart_stowignore_body("DEFAULT\n", &dir, &[]).unwrap();
        assert_eq!(body, "DEFAULT\n");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn build_smart_stowignore_body_single_detected_appends_relative() {
        let dir = make_tempdir("smart-single");
        let abs = dir.join("session-data/foo.tmp");
        let body = build_smart_stowignore_body(
            "DEFAULT\n",
            &dir,
            &[abs.to_string_lossy().to_string()],
        ).unwrap();
        assert!(body.starts_with("DEFAULT\n"));
        assert!(body.contains("# Auto-detected from secret-scan\n"));
        assert!(body.contains("session-data/foo.tmp\n"));
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn build_smart_stowignore_body_multiple_detected() {
        let dir = make_tempdir("smart-multi");
        let p1 = dir.join("session-data/a.tmp");
        let p2 = dir.join("plugins/marketplaces/everything-claude-code/mcp-configs/x.json");
        let body = build_smart_stowignore_body(
            "DEFAULT\n",
            &dir,
            &[p1.to_string_lossy().to_string(), p2.to_string_lossy().to_string()],
        ).unwrap();
        assert!(body.contains("session-data/a.tmp\n"));
        assert!(body.contains("plugins/marketplaces/everything-claude-code/mcp-configs/x.json\n"));
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn build_smart_stowignore_body_rejects_outside_claude_dir() {
        let dir = make_tempdir("smart-outside");
        // Use an absolute path that cannot share a prefix with `dir` regardless
        // of platform — both Windows (drive letter) and Unix (root) are covered
        // by picking a sibling of the tempdir's parent.
        let parent = dir.parent().unwrap();
        let outside = parent.join("definitely-not-inside-claude-dir-xyz");
        let err = build_smart_stowignore_body(
            "DEFAULT\n",
            &dir,
    ...
    #[test]
    fn build_smart_stowignore_body_handles_windows_case_mismatch() {
        let dir = make_tempdir("smart-case-mismatch");
        // tempdir 자체의 경로 대소문자를 강제로 다르게 — Windows-only behaviour
        // 시뮬레이션이지만 cross-platform한 함수 자체의 case-insensitive 폴백을 검증.
        let dir_str = dir.to_string_lossy().to_string();
        let upper = dir_str.to_uppercase();   // 전체 대문자 변환
        // upper == dir_str 이면 환경상 차이 없음 — skip
        if upper == dir_str {
            let _ = std::fs::remove_dir_all(&dir);
            return;
        }
        let abs = format!("{upper}{}", if upper.ends_with(std::path::MAIN_SEPARATOR) {""} else {"/"}) + "session-data/x.tmp";
        let body = build_smart_stowignore_body("DEFAULT\n", &dir, &[abs]).unwrap();
        // 정확히 한 entry 추가됐는지 확인 (case 폴백 통과)
        assert!(body.contains("session-data/x.tmp"), "body: {body}");
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn build_smart_stowignore_body_deduplicates() {
        let dir = make_tempdir("smart-dedup");
        let abs = dir.join("session-data/x.tmp").to_string_lossy().to_string();
        let body = build_smart_stowignore_body(
            "DEFAULT\n",
            &dir,
            &[abs.clone(), abs.clone(), abs.clone()],
        ).unwrap();
